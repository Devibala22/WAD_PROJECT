...
const verifyEmail = async (Email, Firstname, createdStudent_id) => {
    try {
        ...
        // Don't throw error, just log it so signup can complete
    }
};
...
const signup = asyncHandler(async (req, res) =>{
    ...
    const newStudent = await student.create({
        ...
        Password,
        Studentdetails:null,
        Isverified: process.env.TEST === 'true' ? true : false
    })
...
const login = asyncHandler(async(req,res) => {
    ...
    const options = {
        httpOnly:true,
        secure: false,     // ✔ allow cookies in localhost
    sameSite: "lax" , 
    path: "/"     // ✔ required for React + fetch + cookies
    };
...
const logout = asyncHandler(async(req,res)=>{
    // Try to get token and clear refresh token if user is authenticated
...
const forgetPassword=asyncHandler(async(req,res)=>{
   ...
   <p>${resetToken}</p>
   <p>Thank you for being a valued member of the EurekaPath community. If you have any questions or need further assistance, please do not hesitate to contact our support team.</p>
   <p>Best regards,</p>
   <p>The EurekaPath Team</p>`
